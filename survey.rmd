...
survey_full <- left_join(survey, eval_list, by = c("title"))
survey_full <- left_join(survey, eval_list %>% select(itemid, title), by = c("recommendation_title" = "title")) %>% 
...
survey_full %>% select(-reference) %>% na.omit() %>% filter(distinct_authors == 3) %>% mutate(same_author = ifelse(author.x == recommendation_author,"same author", "different author")) %>% ggplot(aes(same_author)) + geom_histogram(stat = "count") + labs(title = "how often is same author chosen if exactly one of same author is available")

survey_full %>% select(-reference) %>% na.omit() %>% filter(distinct_authors == 2) %>% mutate(same_author = ifelse(author.x == recommendation_author,"same author", "different author")) %>% ggplot(aes(same_author)) + geom_histogram(stat = "count") + labs(title = "how often is same author chosen if two one of same author is available")
...
s1 <- left_join(s1, eval_list %>% select(title, itemid), by = c("itemid_rec" = "itemid")) %>% 
  rename(topic = topic.x, topic_rec = topic.y)

s1 <- s1 %>% mutate(same_category = ifelse(topic == topic_rec, "yes", "no"))

s1 %>% filter(is.na(topic_rec)) %>% group_by(recommendation_author) %>% count

s1 %>% ggplot(aes(same_category)) + 
  geom_histogram(stat = "count")
  #labs(title = "choice behavior when 2+ languages are available")
